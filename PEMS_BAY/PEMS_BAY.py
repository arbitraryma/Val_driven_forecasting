...
Same pipeline as your PEMS-SF script, but dataset is a single CSV:
  - first column: timestamp
  - remaining columns: sensor values
  - data recorded every 5 minutes
  - we keep only complete days (288 rows/day)
  - each day is treated as one multivariate time series object
...
def train_mixgru_early_stop(
    ...
    model = MixGRU(input_dim=P, mix_dim=mix_dim, hidden_dim=hidden_dim,
...
def score_val_mse_mae_mean_assigned(cluster_models, labels, X_val, window_size, batch_size, device):
    """Mean VAL MSE/MAE over ALL windows (h=1), routing each sample to its cluster model."""
...
def score_val_huber_mean_assigned(cluster_models, labels, X_val, window_size, batch_size, delta, device):
    """Mean VAL HUBER over ALL windows (h=1), routed."""
    labels = np.asarray(labels, dtype=int)
    K = len(cluster_models)
...
def load_pems_bay_csv_to_days(path: str, ts_col=None, expected_delta="0 days 00:05:00", keep_only_full_days=True):
    ...
    if expected_delta is not None:
        # soft check only
        if str(mode_delta) != str(expected_delta):
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="Forecastability-aware clustering on PEMS-BAY.csv (days-as-series).")
...
if __name__ == "__main__":
    ...
    if in_ipykernel:
        ...
        run(csv_path="/Users/maz0b/Desktop/Forecast/PEMS_BAY/PEMS-BAY.csv", min_cluster_size=10)
        # print("[info] In notebook/ipykernel: call run(csv_path=...) manually.")
    else:
        main()