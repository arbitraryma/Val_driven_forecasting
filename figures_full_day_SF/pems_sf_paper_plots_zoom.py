...
def _get_zoom_window_last_train_plus_val_test(X_full, split_val_start, split_test_start, train_fraction=0.10):
    """
    Show:
      - last `train_fraction` of TRAIN
      - full VAL
      - full TEST
    """
    train_len = split_val_start
    keep_train = max(1, int(np.ceil(train_fraction * train_len)))
    x_start = max(0, split_val_start - keep_train)
    x_end = X_full.shape[1] - 1
...
def plot_single_series_full_day_comparison(
    ...
    # show last 10% of TRAIN + full VAL + full TEST
    x_start, x_end = _get_zoom_window_last_train_plus_val_test(
        X_full=X_full,
    ...
    for rr, ch in enumerate(chosen_channels):
        ...
        ax.plot(
            t_full[x_start:x_end + 1],
            full_series[x_start:x_end + 1, ch],
            linewidth=1.2,
        ...
        # shade only first h predicted region in TEST
