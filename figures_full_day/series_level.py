...
# Fixed representative series and figure format
FIXED_CHOSEN_SERIES = [309, 339, 285, 179]
FIG_EXT = "pdf"
...
def generate_full_day_prediction_figures(
    ...
    # Use fixed representative series instead of automatic ranking
    chosen_series = np.array(FIXED_CHOSEN_SERIES, dtype=int)
    chosen_series = chosen_series[(chosen_series >= 0) & (chosen_series < Xfull_std.shape[0])]
    chosen_series = chosen_series[:n_rep_series]

    fig1 = os.path.join(out_dir, f"representative_prediction_panels_full_day_h{horizon_plot}.{FIG_EXT}")
    ...
    if len(chosen_series) > 0:
        fig2 = os.path.join(out_dir, f"single_series_full_day_h{horizon_plot}.{FIG_EXT}")
    ...
    fig3 = os.path.join(out_dir, f"improvement_distribution_h{horizon_plot}.{FIG_EXT}")
...
def parse_args(argv=None):
    ...
    p.add_argument("--full-day-fig-n-series", type=int, default=4)
